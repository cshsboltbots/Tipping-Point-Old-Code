...
int RunRing(double rotations,int dir){
...
void autonomous(void) {
  ...
  double drive = 1.009;
  Drive(64*drive,64*drive);
  wait(.84,sec);
  RunGoal(1);
  wait(.1,sec);
  Drive(-53.5*drive,-53.5*drive);
  RunRing(10,-1);
  wait(.75,sec);
  Drive(-135*angle,135*angle);
  Drive(14*drive,14*drive);
  RunGoal(0);
  Drive(-8,-8);
  Drive(206*angle,-206*angle);
  //END
  //END
}
