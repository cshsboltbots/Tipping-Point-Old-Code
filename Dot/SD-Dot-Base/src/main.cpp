...
int Drive(){
  while (true){
    ...
    //Driver Control//
    //////////////////
    if(Controller1.ButtonA.pressing() == 1 && Controller1.ButtonLeft.pressing() == 1){
      Left.stop(hold);
      Right.stop(hold);
    }
    else if(Controller1.ButtonA.pressing() == 1 && Controller1.ButtonRight.pressing() == 1){
      Left.stop(coast);
      Right.stop(coast);
    }
    else{
    Left.spin(vex::directionType::fwd, (Controller1.Axis3.value()*1.5 + Controller1.Axis1.value()/2), vex::velocityUnits::pct);
    Right.spin(vex::directionType::fwd, (Controller1.Axis3.value()*1.5 - Controller1.Axis1.value()/2), vex::velocityUnits::pct);
    }
    wait(20, msec);
  }
}
