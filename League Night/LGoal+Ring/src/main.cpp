...
//Zero for forward, one for reverse
...
int RunGoal(int dir,int w,double ro){
  if (dir == 1){
    Goal.setVelocity(100,pct);
    Goal.startRotateFor(forward,ro, rev);
    if(w == 1){
      waitUntil(Goal.isDone());
    }
  }
  else {
    Goal.setVelocity(100,pct);
    Goal.startRotateFor(forward,-1*ro, rev);
    if(w == 1){
      waitUntil(Goal.isDone());
    }
  }
...
void autonomous(void) {
  ...
  //Drive(-10,10);
  /*
  // Run Ring Lift Forward for 1 second
  RunRing(1,0);
  ...
  // Run Goal Lift Forward for 1 second
  RunGoal(1,0);
...
void usercontrol(void) {
  ...
  while (true){
    ...
    if (Controller1.ButtonR1.pressing() == 1) {
      ...
      Goal.spin(reverse,50,pct);
    }
    else if (Controller1.ButtonL2.pressing() == 1) {
      Goal.spin(forward,50,pct);
    }
    else {
      Ring.stop();
      Goal.stop(hold);
