...
//Zero for forward, one for reverse
int RunGoal(int dir,int w){
  if (dir == 1){
    Goal.setVelocity(100,pct);
    Goal.startRotateFor(reverse, 4, rev);
    if(w == 1){
...
void usercontrol(void) {
  ...
  while (true){
    ...
    else if (Controller1.ButtonL1.pressing() == 1 && Goal.isDone()) {
      Goal.setVelocity(100,pct);
      Goal.startRotateFor(forward, 4, rev);
    }
    else if (Controller1.ButtonL2.pressing() == 1 && Goal.isDone()) {
      Goal.setVelocity(100,pct);
      Goal.startRotateFor(reverse, 4, rev);
    }
    else {
