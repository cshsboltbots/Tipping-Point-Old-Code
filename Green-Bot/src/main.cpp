...
//int postition - (0) up | (1) middle | (2) down
/*
int RunGoal(int position){
  if (position == 0){
    Goal.startSpinTo(0,rev,100, vex::velocityUnits::pct);
    waitUntil(Goal.isDone());
  }
  else if(position == 1){
    Goal.startSpinTo(2.45,rev,100, vex::velocityUnits::pct);
    waitUntil(Goal.isDone());
  }
  else if(position == 2){
    Goal.startSpinTo(3.5,rev,100, vex::velocityUnits::pct);
    waitUntil(Goal.isDone());
  }
  return 1;
...
void usercontrol(void) {
  ...
  while (true){
    ...
    /*
      ...
      Goal.spin(reverse,100,pct);
    }
    else if (Controller1.ButtonR2.pressing() == 1) {
      Goal.spin(forward,100,pct);
    }
